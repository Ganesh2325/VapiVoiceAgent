...
/**
 * Honest MOCK email adapter. Never claims a real message was sent.
 */
@Service
public class MockEmailProvider implements EmailProvider {
    ...
    public MockEmailProvider(ProviderBinding binding) {
        this.binding = binding != null ? binding : ProviderBinding.mockDefault();
    }

    @Autowired
    public MockEmailProvider(@Autowired(required = false) VoiceOsProperties properties) {
        this(properties != null && properties.providers() != null
                ? properties.providers().email()
    ...
    public String getDescription() {
        return "[MOCK] Simulated email send. Does not deliver mail.";
    }
    ...
    public Set<ProviderCapability> capabilities() {
        return Set.of(ProviderCapability.EMAIL_SEND);
    }
    ...
    public boolean supportsOperation(String operation) {
        ...
        return normalized.contains("email") || normalized.contains("send");
    }
    ...
    public ProviderResult send(ProviderRequest request) {
        ...
        String recipient = safe.paramAsString("recipient");
        if (recipient == null || recipient.isBlank()) {
            return ProviderResult.failure(getName(), ProviderMode.MOCK, "send",
                    ProviderErrorCode.INVALID_REQUEST, "Email recipient is required", elapsed(startMs));
        }
        ...
        return ProviderResult.success(getName(), ProviderMode.MOCK, "send", data,
                "MOCK: email send simulated for " + recipient, elapsed(startMs));
    }
